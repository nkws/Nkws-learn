...
function buildCh3() {
  return shuffle([
    ...
    { q: "头发 长在哪里？", a: "头上", choices: ["手上", "头上", "脚上"], explain: "头发（tóufa）名字里就有头字，它一根一根长在头顶上，不会长在手上或脚上。" },
  ]).map((item) => ({ question: item.q, answer: item.a, choices: shuffle([...item.choices]), explain: item.explain }));
}
...
function buildCh4() {
  return shuffle([
    ...
    { q: "牛 可以给我们什么？", a: "牛奶", choices: ["鸡蛋", "牛奶", "蜂蜜"], explain: "牛（niú）名字里就有牛字，妈妈牛会产奶，所以牛给我们的是牛奶。" },
    { q: "鸡 会下什么？", a: "鸡蛋", choices: ["牛奶", "鸡蛋", "面包"], explain: "鸡（jī）名字里就有鸡字，母鸡会下蛋，所以鸡会下的是鸡蛋。" },
  ]).map((item) => ({ question: item.q, answer: item.a, choices: shuffle([...item.choices]), explain: item.explain }));
}
